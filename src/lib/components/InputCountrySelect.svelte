...
<script lang="ts" generics="T extends Record<string, unknown>">
	...
	const defaultClass =
		'w-full block py-3 px-4 text-gray-900 placeholder-gray-400 border border-gray-300 focus:border-gray-300 rounded-md';
...
{#if label}
	<label for={field} class={cn(defaultLabelClass, labelClass)} data-label={dataAttribute}
		>{label}</label
	>
{/if}
<select
	...
	{...$constraints}
	{...rest}
	data-select-input={dataAttribute}
