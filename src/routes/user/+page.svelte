	...
	<div class="mx-auto max-w-4xl space-y-8 p-6">
		...
		<div class="rounded-lg bg-primary-700 p-6 shadow-md">
			<div class="mb-6 flex items-center justify-between">
				<h1 class="text-3xl font-bold">{m.customer_profile()}</h1>
				{#if !isEditing}
					<button
			...
			{#if isEditing}
				<form onsubmit={handleSubmit} class="space-y-6">
					...
					<div>
						<h2 class="mb-4 text-xl font-semibold">{m.personal_information()}</h2>
						<div class="grid gap-4 md:grid-cols-2">
							<label class="block">
								<span class="text-sm font-medium">{m.first_name()}</span>
								<input
									type="text"
							...
							<label class="block">
								<span class="text-sm font-medium">{m.last_name()}</span>
								<input
									type="text"
							...
							<label class="block">
								<span class="text-sm font-medium">{m.phone()}</span>
								<input
									type="tel"
					...
					{#if addressFormData}
						<div>
							<h2 class="mb-4 text-xl font-semibold">{m.default_address()}</h2>
							<div class="grid gap-4 md:grid-cols-2">
								<label class="block">
								...
								<label class="block">
									<span class="text-sm font-medium">{m.street_line_1()}</span>
									<input
										type="text"
								...
								<label class="block">
									<span class="text-sm font-medium">{m.street_line_2()}</span>
									<input
										type="text"
								...
								<label class="block">
									<span class="text-sm font-medium">{m.city()}</span>
									<input
										type="text"
								...
								<label class="block">
									<span class="text-sm font-medium">{m.postal_code()}</span>
									<input
										type="text"
			...
			{:else if user}
				<div class="grid gap-6 md:grid-cols-2">
					<div>
						<h2 class="mb-4 text-xl font-semibold">{m.personal_information()}</h2>
						<div class="space-y-2">
							<p><span class="font-medium">{m.name()}:</span> {user.firstName} {user.lastName}</p>
							<p><span class="font-medium">{m.email()}:</span> {user.emailAddress}</p>
							<p><span class="font-medium">{m.phone()}:</span> {user.phoneNumber || m.not_provided()}</p>
						</div>
					</div>
		...
		{#if user?.orders}
			<div class="rounded-lg bg-primary-700 p-6 shadow-md">
				<h2 class="mb-6 text-2xl font-bold">{m.order_history({ count: user.orders.totalItems })}</h2>
					...
					{#each user.orders.items as order}
						<div class="rounded-lg border bg-primary-400 p-4 transition-colors">
							<div class="mb-4 flex items-start justify-between">
								<div>
									<p class="font-medium">{m.order_number({code:order.code})}</p>
									<p class="text-sm">
