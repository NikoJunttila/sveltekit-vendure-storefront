<script lang="ts">
	...
	import { updateCustomer, updateCustomerAddress, CreateCustomerAddress, GetCustomer } from '$src/lib/vendure';
	import type {
		UpdateCustomerInput,
		UpdateAddressInput,
		CreateAddressInput
				...
				{:else}
					<div class="rounded-lg bg-gray-50 dark:bg-slate-300 p-4">
						<div class="space-y-4">
							...
							{#if isAddingAddress}
								<form onsubmit={async () => await createAddress(newAddressFormData)} class="space-y-4">
									<div class="grid gap-4 sm:grid-cols-2">
										<div class="space-y-2">
											<label class="block text-sm font-medium text-gray-700">{m.full_name()}
											<input
												...
												class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-gray-900 sm:text-sm"
												required
											/>
										</label>
										</div>
